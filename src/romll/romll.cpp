...
void ROMLL::build_graph_from_onnx(const onnx::ModelProto &model,
  ...
  for (const auto &node : g.node()) {
    ...
    std::string lbl = graph.generate_block_label(op);
...
void ROMLL::run_debug_inference() {
  try {
    ...
    std::unordered_map<Block *, TensorData> computed;
    ...
    while (!queue.empty()) {
      ...
      if (bname == "PortInput") {
        TensorData td;
        for (int d : cur->shape_dims)
          td.shape.push_back((int64_t)d);
        if (td.shape.empty())
          td.shape = {(int64_t)cur->values.size()};
        td.values = cur->values;
        computed[cur] = td;

        cur->debug_output_values = td.values;
        cur->debug_output_shape = td.shape;
        cur->has_debug_values = true;

      } else if (bname != "PortOutput" && !cur->inputs.empty()) {
        bool all_ready = true;
        for (auto &inp : cur->inputs) {
          if (!computed.count(inp.block)) {
            all_ready = false;
            break;
          }
        }
        ...
        if (all_ready) {
          ...
          if (mini.SerializeToString(&mini_bytes) && !mini_bytes.empty()) {
            try {
              ...
              Ort::Session mini_sess(env, mini_bytes.data(), mini_bytes.size(),
                                     mini_opts);
              ...
              for (size_t i = 0; i < cur->inputs.size(); i++) {
                TensorData &td = computed.at(cur->inputs[i].block);
              ...
              auto results =
                  mini_sess.Run(run_opts, in_ptrs.data(), in_vals.data(),
              ...
              computed[cur] = out_td;
              cur->debug_output_values = out_td.values;
              cur->debug_output_shape = out_td.shape;
              cur->has_debug_values = true;
            } catch (...) {
            }
          }
    ...
    graph.push_notification("Debug: unexpected failure", true);
  }
}
